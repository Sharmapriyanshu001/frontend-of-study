...
function AdminLayout() {
  ...
  const navItems = [
    ...
    { name: 'Manage Notices', path: '/admin/manage-notices' },
  ];

  const handleLogout = () => {
    localStorage.removeItem('token');
    localStorage.removeItem('role');
    toast.info('Admin logged out securely.');
    navigate('/');
  };
      ...
      <nav className="bg-white border-b border-purple-100 sticky top-0 z-50 shadow-sm">
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center gap-8">
              ...
              <div className="flex-shrink-0 flex items-center">
                <Link to="/admin" className="text-xl font-bold bg-gradient-to-r from-purple-600 to-purple-800 bg-clip-text text-transparent tracking-tight">
                  Study-material-app
                </Link>
              </div>
        ...
        <div className={`${isMenuOpen ? 'block' : 'hidden'} md:hidden bg-white border-t border-purple-50`}>
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <button
              onClick={handleLogout}
              className="block w-full text-left px-3 py-2 rounded-md text-base font-bold text-purple-600 hover:bg-purple-50"
            >
              Logout
            </button>
          </div>
